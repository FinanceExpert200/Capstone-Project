...
export default class Trading {
  ...
  static async compareMovingAverage(currentPrice, movingAverage, ticker, date) {
    ...
    this.movingAveragePercentageArray.push({
      ticker: ticker,
      percentage: percentIncrease,
    });

    if (percentIncrease < -10) {
      this.unfilteredHistoricalData.push({
        ticker: ticker,
        type: "buy",
        date: date,
        price: currentPrice,
        buyingPower: this.botBuyingPower,
        accountValue: this.botAccountValue,
      });
      if (this.botBuyingPower > currentPrice) {
        this.botBuyingPower -= currentPrice;
        this.ownedStock += 1;
        this.botAccountValue =
          this.botBuyingPower + this.ownedStock * currentPrice;
        ...
        this.botTransactionHistory.push({
          ticker: ticker,
          type: "buy",
          date: date,
          price: currentPrice,
  ...
  static async calculateIndividualShare(ticker, budgetPerStock) {
    ...
    const oneYearHistoricalData = await this.fetchHistoricalData(
      ticker,
      startDate
    );
    ...
    while (rightPointer != oneYearHistoricalData.length) {
      this.calculateMovingAverage(
        oneYearHistoricalData,
        oneYearHistoricalData[leftPointer].date,
        oneYearHistoricalData[rightPointer].date,
        ticker
      );

      if (rightPointer == Math.round(oneYearHistoricalData.length / 4)) {
        console.log(
          "three months hit, account value is ",
          this.botAccountValue
        );
  ...
  static async calculateDisplayedProfit(budget, tickerArray) {
    ...
    for (let i = 0; i < tickerArray.length; i++) {
      let profit = await this.calculateIndividualShare(
        tickerArray[i],
        budgetPerStock
      );
  ...
  static getAccountValue() {
    ...
    return [
      this.totalThreeMonthProfit,
      this.totalSixMonthProfit,
